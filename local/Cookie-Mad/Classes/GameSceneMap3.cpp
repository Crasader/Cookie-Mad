...
#define SPEED_BACIC_C2 33
#define SPEED_BACIC_C3 37
...
Scene* GameSceneMap3::createScene()
{
	// 'scene' is an autorelease object
	auto scene = Scene::createWithPhysics();

	// 'layer' is an autorelease object
	auto layer = GameSceneMap3::create();
...
bool GameSceneMap3::init()
{
	...
	clickReloadBtt->setPosition(Vec2(clickBackBtt->getPositionX()
		...
		clickSprite->getPositionY()));
	clickReloadBtt->addTouchEventListener(CC_CALLBACK_2(GameSceneMap3::reloadEvent, this));
	clickReloadBtt->setScaleX(SceneManager::Instance()->getXScale());
	clickReloadBtt->setScaleY(SceneManager::Instance()->getYScale());
	this->addChild(clickReloadBtt, 200);
	...
	_score = 10;
	_scoreStr = __String::createWithFormat(" %i / %i", _score, FINISH_SCORE_50);
	...
	shadow1->setPosition(Vec2(x1, y3));
	...
	// position the sprite on the center of the screen
	//float hei = visibleSize.height  + origin.y;
	//float ni = -clickSprite->getContentSize().height * SceneManager::Instance()->getYScale();
	//float heifi = 0.166 * SceneManager::Instance()->getSizeOfSquare();

	shadow2->setPosition(Vec2(x2, y2));
	...
	shadow3->setPosition(Vec2(x3, y1));
	...
	////////////////// COOKIES
	cook1_start = Vec2(SceneManager::Instance()->getSizeOfSquare() + far_dis, shadow1->getPositionY());
	cook1_end = Vec2(-far_dis, shadow1->getPositionY());
	...
	cook2_start = Vec2(shadow1->getPositionX() - far_dis, shadow3->getPositionY() - far_dis);
	cook2_end = Vec2(shadow3->getPositionX() + far_dis, shadow1->getPositionY() + far_dis);
	...
	cook3_start = Vec2(shadow1->getPositionX() - far_dis, shadow3->getPositionY());
	cook3_end = Vec2(shadow3->getPositionX() + far_dis, shadow3->getPositionY());
	_cookie3 = Sprite::create("normal/cokie.png");
	_cookie3->setPosition(Vec2(shadow2->getPositionX(), shadow3->getPositionY()));
	...
	touchListener->onTouchBegan = CC_CALLBACK_2(GameSceneMap3::onTouchBegan, this);
...
bool GameSceneMap3::onTouchBegan(cocos2d::Touch *touch, cocos2d::Event *event)
{
	if (_isWon)
		return _isWon;
...
//#if (CC_TARGET_PLATFORM == CC_PLATFORM_IOS)
//        CocosDenshion::SimpleAudioEngine::getInstance()->playEffect("sound/touch.wav",false);
//#else
...
//#endif
		if (_numberOfClick % 5 == 0)
			SwapInt(_pixelPerUpdateC1, _pixelPerUpdateC3);
		else if (_numberOfClick % 3 == 0)
			SwapInt(_pixelPerUpdateC1, _pixelPerUpdateC2);
		else if (_numberOfClick % 2 == 0)
			SwapInt(_pixelPerUpdateC2, _pixelPerUpdateC3);

		//_pixelPerUpdateC1 = SPEED_BACIC_C1 * SceneManager::Instance()->getYScale();
		//_pixelPerUpdateC2 = SPEED_BACIC_C2 * SceneManager::Instance()->getYScale();
		//_pixelPerUpdateC3 = SPEED_BACIC_C3 * SceneManager::Instance()->getYScale();
		...
		if (_score >= FINISH_SCORE_50)
		{
			...
			clickBackNotiBtt->setPosition(Vec2(
				...
				, hi_screen));
			clickBackNotiBtt->addTouchEventListener(CC_CALLBACK_2(GameSceneMap3::reloadEvent, this));
			...
			clickNextNotiBtt->setPosition(Vec2(
				...
				+ congraSprite->getContentSize().width  * SceneManager::Instance()->getXScale() * 0.4
				, hi_screen));
			clickNextNotiBtt->addTouchEventListener(CC_CALLBACK_2(GameSceneMap3::backEvent, this));
			...
			SceneManager::Instance()->writeScoreToMap3(_numberOfClick);
...
void GameSceneMap3::update(float dt)
{
	if (!_isClicked)
	{
		...
		if (_cookie2->getPositionX() > cook2_end.x)
			_cookie2->setPosition(cook2_start);
		else{
			_cookie2->setPositionX(_cookie2->getPositionX() + _pixelPerUpdateC3);
			_cookie2->setPositionY(_cookie2->getPositionY() + _pixelPerUpdateC3);
		}

		if (_cookie3->getPositionX() > cook3_end.x)
			_cookie3->setPosition(cook3_start);
		else{
			_cookie3->setPositionX(_cookie3->getPositionX() + _pixelPerUpdateC2);
		}
	}
}
void GameSceneMap3::reloadEvent(Ref* pSender, cocos2d::ui::Widget::TouchEventType eEventType)
{
    if (SceneManager::Instance()->isAdvAvaiable())
    {
...
void GameSceneMap3::backEvent(Ref* pSender, cocos2d::ui::Widget::TouchEventType eEventType)
{
	if (SceneManager::Instance()->isAdvAvaiable())
	{
#if (CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID || CC_TARGET_PLATFORM == CC_PLATFORM_IOS)
...
GameSceneMap3::~GameSceneMap3()
{

}

#if (CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID)
void GameSceneMap3::onKeyReleased(EventKeyboard::KeyCode keyCode, Event* event)
{
	if(SceneManager::Instance()->isAdvAvaiable())
	{
#if (CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID || CC_TARGET_PLATFORM == CC_PLATFORM_IOS)
