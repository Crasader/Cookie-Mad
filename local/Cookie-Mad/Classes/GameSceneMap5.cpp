...
#endif
#define SPEED_BACIC_C1 35
#define SPEED_BACIC_C2 32
#define SPEED_BACIC_C3 28
#define SPEED_BACIC_C4 37
#define SPEED_BACIC_C5 48
...
Scene* GameSceneMap5::createScene()
{
	// 'scene' is an autorelease object
	auto scene = Scene::createWithPhysics();

	// 'layer' is an autorelease object
	auto layer = GameSceneMap5::create();
...
bool GameSceneMap5::init()
{
	...
	_pixelPerUpdateC5 = SPEED_BACIC_C5 * xspeed;
	...
	clickBackBtt->addTouchEventListener(CC_CALLBACK_2(GameSceneMap5::backEvent, this));
	...
	clickReloadBtt->setPosition(Vec2(clickBackBtt->getPositionX()
		...
		clickSprite->getPositionY()));
	clickReloadBtt->addTouchEventListener(CC_CALLBACK_2(GameSceneMap5::reloadEvent, this));
	...
	_score = 10;
	_scoreStr = __String::createWithFormat(" %i / %i", _score, FINISH_SCORE_100);
	...
	shadow5 = Sprite::create("normal/coki_shadow.png");
	shadow5->setPosition(Vec2(x2, y2));
	shadow5->setScaleX(SceneManager::Instance()->getXScale());
	shadow5->setScaleY(SceneManager::Instance()->getYScale());
	this->addChild(shadow5, 9);
	_labelScore5 = Label::createWithTTF("", "fonts/Marker Felt.ttf", FONT_SIZE_TEXT * SceneManager::Instance()->getYScale());
	//_scoreLabel->setScale(scaleLbl, scaleLbl);
	_labelScore5->setColor(Color3B::GREEN);
	_labelScore5->setPosition(Vec2(shadow5->getPositionX() + shadow5->getContentSize().width * SceneManager::Instance()->getXScale() * 0.5,
		shadow5->getPositionY() + shadow5->getContentSize().height * SceneManager::Instance()->getYScale() * 0.5));
	this->addChild(_labelScore5, 90);
	...
	cook5_start = Vec2(x1 - far_dis, y1 - far_dis);
	cook5_end = Vec2(x3 + far_dis, y3 + far_dis);
	_cookie5 = Sprite::create("normal/cokie.png");
	_cookie5->setPosition(cook5_start);
	_cookie5->setScaleX(SceneManager::Instance()->getXScale());
	_cookie5->setScaleY(SceneManager::Instance()->getYScale());
	this->addChild(_cookie5, 33);


	auto touchListener = EventListenerTouchOneByOne::create();
	touchListener->setSwallowTouches(true);
	touchListener->onTouchBegan = CC_CALLBACK_2(GameSceneMap5::onTouchBegan, this);
...
bool GameSceneMap5::onTouchBegan(cocos2d::Touch *touch, cocos2d::Event *event)
{
	if (_isWon)
		return _isWon;
	...
	if (_isClicked)
	{
        if (SceneManager::Instance()->isGoodMusic()) {
            CocosDenshion::SimpleAudioEngine::sharedEngine()->playEffect("sound/touch.wav",false);
        }
		if (_numberOfClick % 7 == 0)
			SwapInt(_pixelPerUpdateC1, _pixelPerUpdateC3);
		else if (_numberOfClick % 5 == 0)
			SwapInt(_pixelPerUpdateC2, _pixelPerUpdateC5);
		else if (_numberOfClick % 3 == 0)
			SwapInt(_pixelPerUpdateC3, _pixelPerUpdateC2);
		else if (_numberOfClick % 2 == 0)
			SwapInt(_pixelPerUpdateC4, _pixelPerUpdateC1);
		...
		///5
		///
		_currentDis = distance2Point(_cookie5->getPosition(), shadow5->getPosition());
		if (_currentDis > _bigPointDis){
			_score--;
			_labelScore5->setString("-1");
			_labelScore5->setColor(Color3B::RED);
		}
		else if (_currentDis < _correctPointDis){
			_score += 3;
			_labelScore5->setString("+3");
			_labelScore5->setColor(Color3B::BLUE);
		}
		else if (_currentDis < _shortPointDis){
			_score += 2;
			_labelScore5->setString("+2");
			_labelScore5->setColor(Color3B::GREEN);
		}
		else{
			_score++;
		...
		if (_score >= FINISH_SCORE_100)
		{
			...
			clickBackNotiBtt->setPosition(Vec2(
				...
				, hi_screen));
			clickBackNotiBtt->addTouchEventListener(CC_CALLBACK_2(GameSceneMap5::reloadEvent, this));
			...
			clickNextNotiBtt->setPosition(Vec2(
				...
				+ congraSprite->getContentSize().width  * SceneManager::Instance()->getXScale() * 0.4
				, hi_screen));
			clickNextNotiBtt->addTouchEventListener(CC_CALLBACK_2(GameSceneMap5::backEvent, this));
			...
			SceneManager::Instance()->writeScoreToMap5(_numberOfClick);
		...
		auto actionFade5 = FadeOut::create(3);
		...
		_labelScore5->runAction(actionFade5);
	}
	return _isClicked;
}

void GameSceneMap5::update(float dt)
{
	if (!_isClicked)
	{
		...
		if (_cookie5->getPositionX() > cook5_end.x)
			_cookie5->setPosition(cook5_start);
		else{
			_cookie5->setPositionX(_cookie5->getPositionX() + _pixelPerUpdateC5);
			_cookie5->setPositionY(_cookie5->getPositionY() + _pixelPerUpdateC5);
		}

	}
}
void GameSceneMap5::reloadEvent(Ref* pSender, cocos2d::ui::Widget::TouchEventType eEventType)
{
    if (SceneManager::Instance()->isAdvAvaiable())
    {
#if (CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID || CC_TARGET_PLATFORM == CC_PLATFORM_IOS)
...
void GameSceneMap5::backEvent(Ref* pSender, cocos2d::ui::Widget::TouchEventType eEventType)
{
	if (SceneManager::Instance()->isAdvAvaiable())
	{
#if (CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID || CC_TARGET_PLATFORM == CC_PLATFORM_IOS)
...
GameSceneMap5::~GameSceneMap5()
{

}

#if (CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID)
void GameSceneMap5::onKeyReleased(EventKeyboard::KeyCode keyCode, Event* event)
{
	if (SceneManager::Instance()->isAdvAvaiable())
	{
#if (CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID || CC_TARGET_PLATFORM == CC_PLATFORM_IOS)
